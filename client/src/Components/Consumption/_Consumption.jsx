...
const Consumption = () => {
	...
	const [ roomData, roomLoading,  getRooms ] = useMutation(`/api/ontology/${building}/rooms`);
	const { data: dataFloors, loading: floorsLoading } = useQuery('/api/ontology/'+building.toString()+'/floors');
	const floors = dataFloors?.floors;

	useEffect(() => {
		if (!floorsLoading && floors) {
			const toPass = floors[0]
			const getRoomsData = async () => {
				try {
					const res = await axios.get(`${import.meta.env.VITE_API_URL}/api/ontology/CREWW/rooms`, {
						params: {toPass}
					});
	...
	}, [floorsLoading, floors]);

	{rooms?.map((room) => (
		console.log(room)
	))}

	const onSelectFloor = async (floorId) => {
		getRooms(floorId);
	}
